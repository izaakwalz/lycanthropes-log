...
const corelations = (phinumber, event) => {
    const activityResult = phinumber >= 0.02 ? `${event} will completly turn you to a warewolf` : 
    phinumber >= 0.01 ? `${event} shows waek positive signs of a warewolf` : 
    phinumber <= -0.01 ? `${event} have a negative effect might turn to a warewolf ` :
    phinumber <= 0.00 ? `${event} have no relation to a warewolf` : `${event} makes you warm blooded`
...
const phiTable = (events = activityEvents(EventLog), activities) => {
  ...
  activities =  events.map(event => {
      const days = table(event, EventLog).toString()
        return {event, days, corelations: phi(table(event, EventLog)), warewolf: corelations(phi(table(event, EventLog)), event)}
    })
